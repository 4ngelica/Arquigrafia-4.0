...
<div class="item h<?php echo $size; ?>">
    ...
    <a href='{{ URL::to("/photos/{$photo->_id}") }}'>
      @if ($photo->type == "video")
    ...
    <div class="item-title">
      ...
      @if (Auth::check() && $photo->institution_id !="" && Session::get('institutionId') == $photo->institution_id)
        ...
        <a id="title_edit_button" href="{{ URL::to('/photos/' . $photo->_id . '/editInstitutional')}}" title="Editar imagem"></a>
      @elseif (Auth::check() && Auth::id() == $photo->user_id && $photo->institution_id == "" &&  !Session::get('institutionId'))
        <a id="title_delete_button" class="title_delete photo" href="{{ URL::to('/photos/' . $photo->_id) }}" title="Excluir imagem"></a>
        <a id="title_edit_button" href="{{ URL::to('/photos/' . $photo->_id . '/edit')}}" title="Editar imagem"></a>
      @endif
    </div>
	</div>
</div>
